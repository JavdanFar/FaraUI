...
`data-side`. Some parts also carry a name flag to distinguish identical siblings:
...
- **Select**: `select` (root), `-trigger`, `-dropdown` (panel), `-option` (`data-selected`), `-empty`
- **Combobox**: `combobox` (root), `-trigger` (`data-disabled`), `-chip`, `-search-input`, `-dropdown` (panel), `-option-list`, `-option`, `-empty`
...
- **Rating**: `rating` (root), `-star` (`data-filled`)
