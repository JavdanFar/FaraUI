...
export function getJalaliWeekday(date: JalaliDate): number {
  const jdn = j2d(date.year, date.month, date.day);
  const { gy, gm, gd } = d2g(jdn);
  const nativeDate = new Date(gy, gm - 1, gd);
  return (nativeDate.getDay() + 1) % 7;
}

export function getTodayJalali(): JalaliDate {
  const now = new Date();
  const jdn = g2d(now.getFullYear(), now.getMonth() + 1, now.getDate());
  return d2j(jdn);
}
